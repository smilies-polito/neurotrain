#!/usr/bin/env python3
"""
Paper-reproduction test: ETLP on SHD with a recurrent RSNN.

Reference experiment (Quintana et al., 2024, Section IV / Table 2):
  - Dataset     : SHD (event-based audio, 700 channels, 20 classes)
  - Network     : recurrent ALIF (n_rec=200) — reported accuracy 74.59%

Updated to use recurrent RSNN (RLeaky neurons) instead of feedforward FCSNN
for closer alignment with the paper's recurrent architecture.

Paper hyperparameters retained:
  - tau_v=80 ms → beta ≈ exp(-1/80) ≈ 0.9876
  - update_rate_hz=100 Hz, dt_ms=1.0 → p=0.1 (Algorithm 1 Poisson trigger)
  - Optimizer: Adamax lr=5e-4 (we use Adam as the closest available)
  - T = 100 fixed bins (paper uses full 1 ms bins, ~250–1000 ms recordings)
"""
...
# Network defaults  (paper: n_rec=200, tau_v=80 ms, thr=1.0, recurrent ALIF)
# NOTE: using feedforward FCSNN here — see module docstring.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="ETLP paper-reproduction on SHD (feedforward LIF; paper uses recurrent ALIF)."
    )
...
def run_training(
    *,
    ...
    # SHD: 700 cochlear channels → in_shape=(700,), 20 classes.
    # Paper uses recurrent ALIF; we use recurrent LIF (RLeaky).
    network = RSNN(
        in_shape=(700,),
        num_classes=20,
        ...
        beta=beta,
        threshold=threshold,
        reset_mechanism="subtract",  # soft reset — matches paper
    ).to(device)
