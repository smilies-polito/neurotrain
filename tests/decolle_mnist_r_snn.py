#!/usr/bin/env python3
"""
Unit-test for RSNN + DECOLLETrainer on MNIST.

Tests the DECOLLE online local-learning algorithm on the MNIST digit
classification task using a recurrent SNN (RSNN) with snnTorch RLeaky neurons.

Usage:
    python test_decolle_rsnn_mnist.py
    python test_decolle_rsnn_mnist.py --epochs 2
    python test_decolle_rsnn_mnist.py --optuna-trials 30 --optuna-epochs 15
    python test_decolle_rsnn_mnist.py --epochs 20 --hpc-prints
"""
...
# DECOLLE-specific defaults
G_SCALE = 0.5
BURN_IN = 0
SURROGATE = "boxcar"       # paper-like default
SURROGATE_SCALE = 5.0
DELTA = 0.5                # paper-like boxcar half-width
H_WITH_NOISE = False
OMEGA_STD = 0.5
LAMBDA_U_UPPER = 0.0
LAMBDA_U_LOWER = 0.0
...
OPTUNA_TRIALS = 0
OPTUNA_EPOCHS = 20
STUDY_NAME = "decolle_rsnn_mnist_study"
OPTUNA_STORAGE = ""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal MNIST+RSNN DECOLLE test.")
...
def run_optuna(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def objective(trial):
        ...
        lambda_u_upper = trial.suggest_float("lambda_u_upper", 0.0, 1e-2, log=True)
        lambda_u_lower = trial.suggest_float("lambda_u_lower", 0.0, 1e-2, log=True)
