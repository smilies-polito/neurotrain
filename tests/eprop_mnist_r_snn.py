...
def run_overfit_test(
    ...
    This isolates whether e-prop (eligibility traces + learning signal + readout)
    is actually working.

    Args:
        trainer:      Initialized EpropTrainer (with network inside).
        train_loader: Full training DataLoader.
        device:       Torch device.
        subset_size:  Number of samples to overfit.
        epochs:       Number of epochs to train.
        target_acc:   Threshold to consider test successful.

    Returns:
        dict with final accuracy and success flag.
    """

    print(f"\n[Overfit Test] subset_size={subset_size} epochs={epochs}")

    # ---- Build tiny deterministic subset ----
    ...
    subset_loader = torch.utils.data.DataLoader(
        subset,
        batch_size=subset_size,   # single batch → faster + deterministic
        shuffle=True,
        pin_memory=(device.type == "cuda"),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for data, target in subset_loader:
            data = data.to(device, non_blocking=non_blocking)
            target = target.to(device, non_blocking=non_blocking)
            data = trainer.normalize_sequence(data)
        ...
        loss_val = total_loss / total_samples if total_samples > 0 else 0.0

        print(f"[Overfit] epoch={epoch}/{epochs} loss={loss_val:.4f} acc={acc:.4f}")

        final_acc = acc

        # Early stop if already perfect
        if acc >= target_acc:
            print(f"[Overfit] reached target accuracy ({target_acc}) early.")
            break
