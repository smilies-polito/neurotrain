#!/usr/bin/env python3
"""
Validation test: TP trainer on CIFAR-10 with the LEGACY CIFAR10_VGG9 class.

Counterpart: tp_cifar10_vgg9_unified.py uses the same network architecture via
the new unified VGG9 factory (vgg9_cifar10).  Running both with identical args
should produce identical epoch metrics — this validates that the unified class is
a correct generalization of the old per-dataset class.
...
BETA      = 0.18   # snntorch LIF beta
THRESHOLD = 0.5    # snntorch LIF threshold
...
# TP-specific
ALPHA          = 0.18   # target-path membrane decay α  (Eq 1)
BETA_TRACE     = 0.19   # eligibility trace decay β     (Eq 11-12)
VTH            = 0.5    # target-path spike threshold
SURROGATE_SCALE = 1.0   # ArcTan surrogate scale

INPUT_SHAPE = (3, 32, 32)   # CIFAR-10 RGB

OPTUNA_TRIALS  = 0
OPTUNA_EPOCHS  = 5
STUDY_NAME     = "tp_cifar10_vgg9_legacy"
OPTUNA_STORAGE = ""
...
from networks.vgg9_cifar10 import CIFAR10_VGG9        # ← old per-dataset class
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TP trainer on CIFAR-10 — legacy VGG9.")
...
def run_training(
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # [EVAL] — CIFAR10_VGG9 uses a LeakyIntegrator head that accumulates
        # mem internally. Read spk_rec[-1] at the FINAL timestep only.
        network.eval()
        total   = 0
        correct = 0
        with torch.no_grad():
...
def main() -> None:
    ...
    print(f"[Run] device={device.type}  network=CIFAR10_VGG9 (legacy)")
