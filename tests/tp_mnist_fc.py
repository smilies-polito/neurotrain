#!/usr/bin/env python3
"""
Validation test: TP trainer on MNIST with FCSNN.

Pes et al. (2026) - "Traces propagation: memory-efficient and scalable
forward-only learning in spiking neural networks"

Hyperparameters follow Table 4 (N-MNIST values used as proxy for MNIST).
"""
...
BETA = 0.9          # snntorch membrane decay (for eval dynamics)
...
# TP-specific (Table 4, N-MNIST: α=0.98, β=0.98, vth=1.0 — used as proxy)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TP trainer on MNIST with FCSNN.")
...
def run_training(
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # With out_integrator=True the output neuron has beta=1.0 / threshold=1e9:
        # it never fires, and mem_rec[-1] after T steps = Σ_t W_out·spk_hidden^t
        # which matches the training integrator exactly.  Do NOT accumulate over
        # timesteps — the integrator state already accumulates internally.
        network.eval()
        total = 0
        correct = 0
        with torch.no_grad():
