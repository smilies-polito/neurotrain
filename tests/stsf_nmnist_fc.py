...
N-MNIST loader + STSFNetwork + STSFTrainer (+ optional Optuna).
...
THRESHOLD = 0.3         # Paper: v_th = 0.3 for N-MNIST (lower threshold for higher noise)
HIDDEN_SIZE = 500       # Paper: hidden layer size 500 for N-MNIST
...
STUDY_NAME = "stsf_nmnist_fc"  # Optuna study name.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal N-MNIST+STSF test.")
...
def run_training(
    *,
    ...
    network = FCSNN(
        in_shape=(2, 34, 34),  # N-MNIST: 34x34 with 2 polarity channels
        num_classes=10,
...
def run_optuna(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def objective(trial):
        ...
        hidden_size = trial.suggest_categorical("hidden_size", [300, 500, 700])
