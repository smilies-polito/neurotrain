#!/usr/bin/env python3
"""
OTTT trainer + unified VGG-9 + CIFAR-10 (direct/analog coding, 3×32×32, T=6).

Hyperparameters from Xiao et al. 2022:
  Optimizer: SGD, lr=0.1, momentum=0.9, CosineAnnealingLR
  lambda (loss_lambda): 0.05
  T: 6 (same static frame repeated — constant_input_per_timestep=True)
  beta (tau=2, lambda=0.5): 0.5

Eval: global_linear head — sum spk_rec[-1] across all T timesteps, argmax.
Training uses x_const = data.mean(dim=0) (same frame every step).
"""
...
EPOCHS       = 5
LR           = 0.1        # SGD, Xiao 2022
LOSS_LAMBDA  = 0.05       # CE/MSE interpolation (Xiao 2022 CIFAR-10)
SEED         = 42
DEVICE       = "auto"
HPC_PRINTS   = False
...
def eval_network(network, test_loader, device) -> float:
    """Spike-sum eval with constant input (CIFAR-10 uses direct coding)."""
    network.eval()
    correct = total = 0
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # CIFAR-10 direct coding: all T frames are identical, use the mean.
            x_const = data.mean(dim=0)
            network.reset()
            spk_sum = None
            for _ in range(data.size(0)):
                spk_rec, _ = network(x_const)
                out = spk_rec[-1]
...
def run_optuna(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def objective(trial):
        ...
        loss_lambda = trial.suggest_float("loss_lambda", 1e-3, 0.2,  log=True)
        result = run_training(
