#!/usr/bin/env python3
"""Minimal smoke test for SHDLoader."""
...
def main() -> None:
    ...
    s = "\n".join("".join("1" if v else "." for v in row) for row in (train_data[0, :, :128] > 0).tolist())
    print("\t sample_data:\n" + s)
    ...
    print(f"[TEST] \n\t data.shape={tuple(test_data.shape)} exprected [batch, timesteps, inputs] \n\t data.ndim={test_data.ndim} \n\t dtype={test_data.dtype}")
    ...
    x = test_data[0, 0]  
    print(f"\t sample_data: shape={tuple(x.shape)} min={x.min().item():.3f} max={x.max().item():.3f} mean={x.float().mean().item():.3f} nnz={(x!=0).sum().item()}")
    s = "\n".join("".join("1" if v else "." for v in row) for row in (test_data[0, :, :128] > 0).tolist())
    print("\t sample_data:\n" + s)
    print("BAD VALUES in sample_data!" if not torch.all((x == 0) | (x == 1)) else "OK (all 0/1)")
