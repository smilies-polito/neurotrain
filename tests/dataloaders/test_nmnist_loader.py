#!/usr/bin/env python3
"""Minimal smoke test for NMNISTLoader."""
...
def main() -> None:
    ...
    print(
        f"\t target.shape={tuple(train_target.shape)} \n\t target.ndim={train_target.ndim} \n\t dtype={train_target.dtype}"
    )
    print(f"\t sample_target={train_target[0]}")
    x = train_data[0, 0, 0]
    print(
        f"\t sample_data: shape={tuple(x.shape)} min={x.min().item():.3f} max={x.max().item():.3f} mean={x.float().mean().item():.3f} nnz={(x!=0).sum().item()}"
    )
    x = x.squeeze()
