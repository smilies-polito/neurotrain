...
MNIST loader + FCSNN + OTTTTrainer (+ optional Optuna).
"""
...
STUDY_NAME = "ottt_mnist_fc"  # Optuna study name.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal MNIST+FCSNN OTTT test.")
...
def run_training(
    *,
    ...
    trainer = OTTTTrainer(
        ...
        # OTTTTrainer default would use tau=2 (trace_decay=0.5) from network.tau which FCSNN
...
def run_optuna(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def objective(trial):
        ...
        beta = trial.suggest_float("beta", 0.80, 0.99)
