#!/usr/bin/env python3
"""
Test for ELL-style local learning on ConvSNN + MNIST.

The current project ELLTrainer is written around flattened fully connected /
recurrent stacks. This script keeps the trainer unchanged and provides a
small conv-specific adapter trainer inside the test file so ConvSNN can be
evaluated without touching production code.

Usage:
    python test_ell_conv_snn_mnist.py
    python test_ell_conv_snn_mnist.py --epochs 20 --beta 0.95
"""
...
class FlatAuxClassifier(AuxClassifier):
    """Aux classifier that flattens conv spikes before the linear decoder."""
...
class ELLConvAdapterTrainer(nn.Module):
    """
    Conv-specific adapter that mirrors the current ELLTrainer logic while
    preserving spatial activations through convolutional layers.
    """

    def __init__(self, network: ConvSNN, lr: float = 5e-4, batch_size: int = 100):
        super().__init__()
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, *network.in_shape, device=next(network.parameters()).device)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MNIST + ConvSNN + ELL-style local learning test."
    )
...
def run_training(
    *,
    ...
    trainer = ELLConvAdapterTrainer(network=network, lr=lr, batch_size=batch_size).to(device)
...
def main() -> None:
    ...
    print(f"[ELL Conv test] device={device.type}")
    print(f"[ELL Conv test] beta={args.beta} threshold={args.threshold}")
    print(f"[ELL Conv test] lr={args.lr} decay=÷{args.lr_decay_factor} every {args.lr_decay_every} ep")
    print(f"[ELL Conv test] timesteps={args.timesteps} batch_size={args.batch_size} epochs={args.epochs}")
