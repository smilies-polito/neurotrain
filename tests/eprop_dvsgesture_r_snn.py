#!/usr/bin/env python3
"""
Unit-test: RSNN + EpropTrainer on DVS Gesture.
...
Note: DVS Gesture has large spatial resolution (2×128×128 = 32768 input dims).
      With n_rec=100, trace_in is [B, 100, 32768] per batch — monitor memory.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DVS Gesture + RSNN e-prop test.")
...
def run_training(
    *,
    ...
    # ── Dataset: DVS Gesture, 2×128×128, 11 classes ──
    ...
    # ── Network: in_shape must match DVS Gesture ──
    network = RSNN(
        in_shape=(2, 128, 128),   # 2 polarity channels, 128×128 → flattened = 32768
        num_classes=11,           # 11 gesture classes
...
# -----------------------------------------------------------------------------
# Overfit test — fresh network, dataset, trainer
# -----------------------------------------------------------------------------
def run_overfit_test(
    *,
    ...
    """
    Sanity check: can a FRESH network+trainer overfit a tiny subset?
    Creates everything from scratch — no contamination from main training.
    """
    ...
    # ── Fresh dataset ──
    train_loader, _ = DVSGestureLoader(
        batch_size=batch_size,
        T=timesteps,
        pin_memory=(device.type == "cuda"),
        seed=seed,
