#!/usr/bin/env python3
"""
Test for ELLTrainer + FCSNN on DVS Gesture.

This script evaluates the current ELL-style local learning setup on the
DVS Gesture classification task using an snnTorch fully connected SNN.

Notes:
    - The current ELLTrainer is primarily aligned with rate-coded
      classification and reuses data[0] across timesteps.
    - This script therefore tests the trainer "as is" on DVSGesture,
      without changing trainer behavior.
    - For DVS Gesture, the main path uses FCSNN with input shape 2×128×128
      and 11 output classes.

Usage:
    python test_ell_fc_snn_dvsgesture.py
    python test_ell_fc_snn_dvsgesture.py --epochs 20 --hidden-sizes 512
    python test_ell_fc_snn_dvsgesture.py --optuna-trials 20 --optuna-epochs 8
"""
...
OPTUNA_TRIALS = 0
OPTUNA_EPOCHS = 10
STUDY_NAME = "ell_fc_dvsgesture_study"
OPTUNA_STORAGE = ""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="DVS Gesture + FCSNN + ELL-style local learning test."
    )
...
def main() -> None:
    ...
    print(f"[ELL FC DVSGesture test] device={device.type}")
    print(f"[ELL FC DVSGesture test] hidden_sizes={hidden_sizes} beta={args.beta} threshold={args.threshold}")
    print(f"[ELL FC DVSGesture test] lr={args.lr} decay=÷{args.lr_decay_factor} every {args.lr_decay_every} ep")
    print(f"[ELL FC DVSGesture test] timesteps={args.timesteps} batch_size={args.batch_size} epochs={args.epochs}")
