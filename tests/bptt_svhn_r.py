...
SVHN loader + RSNN + BPTTTrainer (+ optional Optuna).
"""
...
TIMESTEPS = 10         # Number of rate-coding steps produced by the SVHN loader.
NUM_WORKERS = 4         # DataLoader worker processes for SVHN loading.
DATA_ROOT = ""          # Optional SVHN root override; empty string uses the loader default.
...
STUDY_NAME = "bptt_svhn_r"  # Optuna study name.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal SVHN+RSNN BPTT test.")
    ...
    p.add_argument("--timesteps", type=int, default=TIMESTEPS, help="Number of SVHN rate-coding steps.")
...
def run_training(
    *,
    ...
    network = RSNN(in_shape=(3, 32, 32), num_classes=10, beta=beta, threshold=threshold).to(device)
