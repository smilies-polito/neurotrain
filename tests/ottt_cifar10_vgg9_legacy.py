#!/usr/bin/env python3
"""
Validation test: OTTT trainer on CIFAR-10 with the LEGACY OTTT_VGG9_CIFAR10 class.

Counterpart: ottt_cifar10_vgg9_unified.py uses the same architecture via the new
unified VGG9 factory (vgg9_ottt_cifar10).  Running both with identical args should
produce identical epoch metrics — this validates that the unified class is a correct
drop-in replacement for the old per-dataset class.
...
Architecture note:
  OTTT_VGG9_CIFAR10 has a plain Linear head (no LI). During eval, logits are
  accumulated over all T timesteps before argmax (standard for non-LI heads).
"""
...
OPTUNA_TRIALS  = 0
OPTUNA_EPOCHS  = 5
STUDY_NAME     = "ottt_cifar10_vgg9_legacy"
OPTUNA_STORAGE = ""
...
from networks.vgg9_ottt_cifar10 import OTTT_VGG9_CIFAR10   # ← old per-dataset class
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OTTT trainer on CIFAR-10 — legacy VGG9.")
...
def run_training(
    *,
    ...
    # Set the static-input flag (not in the legacy class's __init__) so OTTTTrainer
    # enables loss_lambda=0.05, grad_clip=0.2, sanitize_grads=True — the full OTTT
    # paper recipe for static CIFAR-10.
    network.constant_input_per_timestep = True

    # Dump conv1 and fc weights so the unified test can verify bit-identical init.
    print(f"[init-check] conv1.weight[:2,0,0,0] = {network.conv1.weight[:2,0,0,0].tolist()}")
    print(f"[init-check]   fc.weight[0,:4]      = {network.fc.weight[0,:4].tolist()}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # [EVAL] — plain Linear head: accumulate logits over T timesteps, then argmax.
        network.eval()
        total   = 0
        correct = 0
...
def main() -> None:
    ...
    print(f"[Run] device={device.type}  network=OTTT_VGG9_CIFAR10 (legacy)")
