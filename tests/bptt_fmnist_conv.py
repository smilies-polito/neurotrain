...
Fashion-MNIST loader + ConvSNN + BPTTTrainer (+ optional Optuna).
"""
...
TIMESTEPS = 10         # Number of rate-coding steps produced by the Fashion-MNIST loader.
NUM_WORKERS = 4         # DataLoader worker processes for Fashion-MNIST loading.
DATA_ROOT = ""          # Optional Fashion-MNIST root override; empty string uses the loader default.
...
STUDY_NAME = "bptt_fmnist_conv"  # Optuna study name.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Minimal FashionMNIST+ConvSNN BPTT test.")
    ...
    p.add_argument("--timesteps", type=int, default=TIMESTEPS, help="Number of Fashion-MNIST rate-coding steps.")
...
def run_training(
    *,
    ...
    network = ConvSNN(in_shape=(1, 28, 28), num_classes=10, beta=beta, threshold=threshold).to(device)
