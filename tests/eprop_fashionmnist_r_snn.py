#!/usr/bin/env python3
"""
Unit-test: RSNN + EpropTrainer on Fashion-MNIST.

Dataset:  Fashion-MNIST, 10 classes, 1×28×28 grayscale images.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fashion-MNIST + RSNN e-prop test.")
...
def run_training(
    *,
    ...
    # ── Dataset: Fashion-MNIST, 1×28×28, 10 classes ──
    ...
    # ── Network: in_shape matches Fashion-MNIST ──
    network = RSNN(
        in_shape=(1, 28, 28),   # 1 channel, 28×28 → flattened = 784
        num_classes=10,
...
def run_overfit_test(
    *,
    ...
    # ── Fresh dataset ──
    train_loader, _ = FashionMNISTLoader(
        batch_size=batch_size,
        T=timesteps,
        pin_memory=(device.type == "cuda"),
        seed=seed,
