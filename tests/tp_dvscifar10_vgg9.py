#!/usr/bin/env python3
"""
TP trainer + unified VGG-9 + DVSCIFAR10 (event-based, 2×128×128, T=10).

Hyperparameters from Pes et al. 2026 (DVSCIFAR10 recipe):
  Optimizer: Adam, lr=1e-4, CosineAnnealingLR
  alpha (membrane decay): 0.18
  beta  (trace decay):    0.19
  vth:                    0.5
...
BETA      = 0.18        # VGG-9 LIF membrane decay (Pes 2026 DVSCIFAR10)
THRESHOLD = 0.5

EPOCHS  = 5
LR      = 1e-4
ALPHA   = 0.18          # TP target-path membrane decay (Pes 2026 DVSCIFAR10)
TP_BETA = 0.19          # TP trace decay (Pes 2026 DVSCIFAR10)
VTH     = 0.5
SEED    = 42
DEVICE  = "auto"
HPC_PRINTS = False
...
def run_optuna(args: argparse.Namespace, device: torch.device) -> None:
    ...
    def objective(trial):
        ...
        result = run_training(
            ...
            beta=alpha,  # keep network beta aligned with target-path alpha
