#!/usr/bin/env python3
"""
Validation test: TP trainer on DVS-Gesture with DVSGEST_VGG9.

Pes et al. (2026) - "Traces propagation: memory-efficient and scalable
forward-only learning in spiking neural networks"

Hyperparameters follow Table 4 (DVS-GESTURE: α=0.98, β=0.53, vth=1.0, batch=64).

Note on input resolution: the paper uses 32×32 for DVS-GESTURE; this test uses
the framework's default 128×128. The algorithm is identical — only the S matrix
(projection from n_classes to first-layer spatial size) scales with resolution.
Pass input_shape=(2, 128, 128) to TPTrainer so it can size S correctly.

Note on evaluation: DVSGEST_VGG9 uses a LeakyIntegrator head (no spike, returns
membrane directly). spk_rec[-1] == mem_rec[-1] == accumulated output membrane.
The standard spike-sum eval is therefore exact for this network.
"""
...
BETA = 0.98         # snntorch LIF beta (conv layers)
THRESHOLD = 1.0     # snntorch LIF threshold (conv layers)
...
# TP-specific (Table 4, DVS-GESTURE: α=0.98, β=0.53, vth=1.0)
ALPHA = 0.98            # membrane decay α (Eq 1)
BETA_TRACE = 0.53       # eligibility trace decay β (Eq 11-12)
VTH = 1.0               # spike threshold
SURROGATE_SCALE = 1.0   # ArcTan surrogate scale

# Must match the DVSGestureLoader output resolution
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TP trainer on DVS-Gesture with VGG9.")
...
def run_training(
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # [EVAL] — VGG9 has a LeakyIntegrator head (leak=1.0, no fire).
