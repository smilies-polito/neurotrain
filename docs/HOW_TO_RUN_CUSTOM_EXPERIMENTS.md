...
This means that a flat override such as:
...
A flat experiment override has the highest priority and is the recommended way to force a value regardless of dataset or trainer. To force a value regardless of dataset, place it explicitly in your experiment file and verify it is not shadowed by a higher-priority section.
...
Run a specific subset of trainers on your custom model with non-default hyperparameters. Use benchmarking mode with a `runtime:` override, then refine with a custom experiment file for the combinations that need specific tuning:
...
# Inject into README.md (between <!-- RESULTS_START --> and <!-- RESULTS_END --> markers)
python3 scripts/generate_results.py experiments/my_campaign/ --readme README.md
```
