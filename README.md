...
| 3 | User overrides in experiment file | `model: { beta: 0.95 }` |
| 2 | Trainer-specific section in model YAML | `tp: { head_type: ... }` |
| 1 (highest) | Dataset-specific section in model YAML | `mnist: { input_shape: ... }` |

Dataset specialization always wins, so you can rely on it to enforce input-shape correctness regardless of what a trainer or user override specifies.
...
*Results from campaign `bench_all_no_vgg`. Generated by `scripts/generate_results.py`.*

![NeuroTrain benchmarking results — test accuracy heatmap, algorithms (rows) × architecture–dataset combinations (cols). Campaign: bench_all_no_vgg — 2026-05-01 15:57](docs/figures/campaign_results/bench_all_no_vgg_heatmap.png)
*NeuroTrain — Benchmarking Results · Campaign: `bench_all_no_vgg` · 2026-05-01 15:57*
...
| DECOLLE | 82.3% | 77.8% | — |
| E-prop | — | — | 91.8% |
| ESD_RTRL | 93.6% | 92.8% | 58.0% |
| ETLP | — | 43.4% | — |

### SHD

*Test accuracy, event-based neuromorphic input, 20 classes.*
