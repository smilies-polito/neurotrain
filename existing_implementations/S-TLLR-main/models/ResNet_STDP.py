...
import logging
__all__ = ["dvscifar10_resnet18_stllr", "dvs_resnet18_stllr", "ncaltech101_resnet18_stllr"]
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, g_function="ADD", activation=None, factors=None, dropout=0.1):
        ...
        conv1 = nn.Sequential(OrderedDict([
            ("conv", Conv2dSTLLR(in_channels, out_channels, kernel_size=3, stride=stride, padding=1,
                                 bias=True, leak=0.0, factors=factors, activation=activation)),

             ]))

        conv2 = nn.Sequential(OrderedDict([
            ("conv", Conv2dSTLLR(out_channels, out_channels, kernel_size=3, stride=1, padding=1,
                                 bias=True, leak=0.0, factors=factors, activation=activation)),
        ]))
        ...
        if self.resize_identity:
            self.identity_conv = nn.Sequential(OrderedDict([
                ("conv", Conv2dSTLLR(in_channels, out_channels, kernel_size=1, stride=stride,
                                     padding=0, bias=True, leak=0.0, factors=factors, activation=activation)),
            ]))

    def init_layer(self):
        # self.activ.reset_state()
        self.body.conv1.conv.reset_state()
        self.body.conv2.conv.reset_state()
        # self.body.dropout.reset_state()
        if self.resize_identity:
            self.identity_conv.conv.reset_state()
...
class ResNet18(nn.Module):
    ...
    def _make_layers(self):
        ...
        sew_init_block = nn.Sequential(OrderedDict([
            ("conv", Conv2dSTLLR(self.in_channels, self.channels_n[0], kernel_size=3, stride=1,
                                 padding=1, bias=True, leak=0.0, factors=self.factors, activation=self.activation)),
    ...
    def reset_states(self):
        self.features.init_block.conv.reset_state()
...
def dvs_resnet18_stllr(args, device):
...
def dvscifar10_resnet18_stllr(args, device):
    if args.activation != "LinearSpike":
        activation = gradients.__dict__[args.activation]
        logging.info("Activation used: "+args.activation)
    else:
...
def ncaltech101_resnet18_stllr(args, device):
    if args.activation != "LinearSpike":
        activation = gradients.__dict__[args.activation]
        logging.info("Activation used: "+args.activation)
    else:
