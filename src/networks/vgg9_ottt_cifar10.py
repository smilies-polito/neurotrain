"""
Spiking VGG-9 OTTT variant for CIFAR-10.

Replicates the OTTT-SNN architecture (Xiao et al., NeurIPS 2022) but adapted for CIFAR-10:
  - in_channels=3  (RGB images instead of DVSGesture's 2)
  - num_classes=10 (CIFAR-10 classes instead of DVSGesture's 11)
  - Smaller pooling: pool after blocks 2, 4 (not 6) to preserve spatial info for 32×32 input

Architecture:
  8 conv blocks [64,128,256,256,512,512,512,512].
  Each block: ScaledWSConv2d(bias=True) → LIF → Scale(2.74) → (AvgPool after blocks 2, 4).
  Classifier: AdaptiveAvgPool2d(1,1) → Linear(512, 10) — no LIF in head.

Input per timestep: (B, 3, 32, 32) — rate-coded CIFAR-10 frame.
Spatial reduction: 32×32 → 16×16 → 8×8 → 4×4 (via two pooling layers).
"""
...
# ---------------------------------------------------------------------------
# OTTT VGG-9 for CIFAR-10
# ---------------------------------------------------------------------------

class OTTT_VGG9_CIFAR10(nn.Module):
    """
    Spiking VGG-9 OTTT variant for CIFAR-10 (3-channel RGB, 10 classes).

    Architecture overview
    ---------------------
    8 convolutional blocks with channel progression [64, 128, 256, 256, 512, 512, 512, 512].
    Each block follows the pattern:
        ScaledWSConv2d  →  LIF neuron  →  Scale(2.74)  →  (AvgPool at blocks 2, 4)

    After all conv blocks:
        AdaptiveAvgPool2d(1, 1)  →  flatten  →  Linear(512, 10)

    Spatial resolution over blocks (CIFAR-10 input 32×32):
      Block 1-2: 32×32 → 16×16 (pool after block 2)
      Block 3-4: 16×16 → 8×8   (pool after block 4)
      Block 5-8: 8×8   → 4×4   (final adaptive pool)
    ...
    def __init__(
        ...
        """
        Args:
            in_channels (int): Input channels. CIFAR-10 uses 3 (RGB).
            num_classes (int): Output classes. CIFAR-10 has 10 classes.
        ...
        # Channel progression through 8 conv blocks
        channels = [64, 128, 256, 256, 512, 512, 512, 512]

        # Pool after blocks 2 and 4 (not 6) to preserve spatial info for 32×32 input
        pool_after = {2, 4}
    ...
    def forward(self, x):
        ...
        for i in range(1, self._num_blocks + 1):
            ...
            # Spatial pooling (blocks 2, 4 only)
...
if __name__ == '__main__':
    B, T = 2, 20
    model = OTTT_VGG9_CIFAR10(in_channels=3, num_classes=10)
    model.init_states()
    ...
    print(f'OTTT_VGG9_CIFAR10  output={spk_list[-1].shape}  params={n_params:,}')
